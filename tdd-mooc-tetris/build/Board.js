...
export class Board {
    ...
    #handleKick(attempt, collisionCoordinates) {
        ...
        const centerColumnCollision = attempt.collisionInternalCoordinates(collisionCoordinates).col === 1 && !attempt.toString().includes("I");
        if (centerColumnCollision) {
            // * If center row collides on rotation, kicking can't be performed
            return null;
        }
        return (this.#setWallKick(this.#wallKick(attempt)) ||
    ...
    #setDoubleFloorKick(attempt) {
        ...
        const forbiddenDoubleFloorKick = attemptIsHorizontalI || this.#nextRowIsEmpty() || this.#hitsImmobile(doubleFloorKick);
